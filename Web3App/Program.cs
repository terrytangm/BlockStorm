    ...
    class Program
    {
        ...
        private static async Task DisplayPoolsAndTokens_UniswapV2()
        {
            ...
            for (int i = 0; i < allPairsLength; i++)
            {
                ...
                try
                {
                    token0Symbol = await token0ContractHandler.QueryAsync<SymbolFunction, string>();
                }
                catch (Exception ex)
                {
                    token0Symbol = "N/A";
                ...
                try
                {
                    token1Symbol = await token1ContractHandler.QueryAsync<SymbolFunction, string>();
                }
                catch (Exception ex)
                {
                    token1Symbol = "N/A";
                ...
                balanceOfFunction.ReturnValue1 = pairAddress;
                bool processBalanceDecimals = false;
                StringBuilder stringBuilderBalance = new StringBuilder();
                try
                {
                    var balanceOfToken0 = await token0ContractHandler.QueryAsync<BalanceOfFunction, BigInteger>(balanceOfFunction);
                ...
                try
                {
                    var token0Decimals = await token0ContractHandler.QueryAsync<DecimalsFunction, byte>();
                ...
                try
                {
                    var balanceOfToken1 = await token1ContractHandler.QueryAsync<BalanceOfFunction, BigInteger>(balanceOfFunction);
                ...
                try
                {
                    var token1Decimals = await token1ContractHandler.QueryAsync<DecimalsFunction, byte>();
